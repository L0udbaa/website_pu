...
class ProgresKeuanganController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        ProgresKeuangan::create($validated);

        return redirect()
            ->route('progres-keuangan.index', $request->input('kegiatan_id') ? Kegiatan::find($request->input('kegiatan_id')) : null)
            ->with('success', 'Progres keuangan berhasil ditambahkan.');
    }

    public function edit(ProgresKeuangan $progresKeuangan, ?Kegiatan $kegiatan = null)
    {
        $kegiatanList = Kegiatan::orderBy('nama_kegiatan')->get();

        return view('progres-keuangan.edit', compact('progresKeuangan', 'kegiatanList', 'kegiatan'));
    }

    public function update(Request $request, ProgresKeuangan $progresKeuangan)
    {
        $validated = $this->validateData($request);
        $validated = $this->hitungOtomatis($validated);
        ...
        return redirect()
            ->route('progres-keuangan.index', $progresKeuangan->kegiatan)
            ->with('success', 'Progres keuangan berhasil diperbarui.');
    }
    ...
    public function destroy(ProgresKeuangan $progresKeuangan)
    {
        ...
        return redirect()
            ->route('progres-keuangan.index', $kegiatan)
            ->with('success', 'Progres keuangan berhasil dihapus.');
    }
    ...
    private function validateData(Request $request): array
    {
        return $request->validate([
            ...
            // rencana_keuangan & deviasi_keuangan TIDAK divalidasi dari input,
            // karena dihitung otomatis di server (lihat hitungOtomatis()).
        ]);
    }
     ...
     * Deviasi (Rp) = Realisasi (Rp) − Rencana (Rp)
     */
    private function hitungOtomatis(array $data): array
    {
        $nilaiKontrak = (float) ($data['nilai_kontrak'] ?? 0);
        $rencanaPersen = (float) ($data['rencana_persen'] ?? 0);
        $realisasiKeuangan = (float) ($data['realisasi_keuangan'] ?? 0);
        $realisasiPersen = (float) ($data['realisasi_persen'] ?? 0);

        $rencanaKeuangan = round($nilaiKontrak * $rencanaPersen / 100, 2);
        $deviasiKeuangan = round($realisasiKeuangan - $rencanaKeuangan, 2);
